...
//TODO: A LA MIERDA LOS ARRAYS INTERMEDIOS Y CAMBIARLO TODO A COMILLA CAMBÁ, meter directamente a total games.
    ...
    for (let url of totalUrls) {
        ...
        let games = await page.evaluate(() => {
            ...
            for (let i = 0; i < divsName.length; i++) {
                pageGames.push({
                    name: divsName[i].innerText,
                    discountPct: divsDiscountPct[i].innerText,
                    newPrice: divsNewPrice[i].innerText,
                    link: divsLink[i].href,
                    img: divsImg[i].src,
                });
            }
